...
if(!$hc) header("Location: ctas-ctes.php");

if(isset($_POST['deb-cred']) && isset($_POST['concepto_credito'])){
	if(trim($_POST['concepto_credito']) <> ""){
		$concepto_credito  = $_POST['concepto_credito'];
		$sql = "insert into conceptos_ctas_ctes (debito_credito, concepto) values('crédito','$concepto_credito');";
...
if(isset($_POST['fecha']) && isset($_POST['importe'])){ # registrar crédito
	...
	$sql = "insert into cuentas_corrientes (hc,fecha,comprobante,debito_credito, concepto, importe, saldo, comentario) ".
	       "values($hc,'$fecha', $comprobante, 'crédito', '$concepto', $importe, $saldo, '$comentario');";
	...
	if ( ! $query = mysql_query($sql, $con)){
			...
			auditoria("crédito cta. cte.", "insertó", mysql_insert_id());
	}
...
bgcolor="#eeeeee" style="margin-left:120px; margin-right:90px; margin-top:65px">
<img style="position:absolute; top:0; left:0; height:70px; width:70px" src="../fotos/<?=$hc . '.jpg'?>">
<h3>Registro de crédito y pagos: <span style="color:#3333FF"><?=residente_nombre_completo($hc)?>	</span><hr></h3>
<form method='post' action='creditos.php' name='wf' onsubmit='return vf_creditos();'>
...
<tr><th  align='right'>concepto crédito *:</th>
<td>

<?
if (isset($_POST['concepto_credito']))
	$cpto_cred =$_POST['concepto_credito'];
else
	$cpto_cred = "";

$sql = "Select concepto, concepto from conceptos_ctas_ctes where debito_credito='crédito' order by concepto;";
selectHTML($sql, 'concepto', 1, '', '', '', $cpto_cred, '')
?>
...
<td>
<input name='importe' value='' size='12'  style='text-align:right'>
</td>
...
Agregar concepto de crédito: 
<input name="concepto_credito" size="30" maxlength="60">
<input type="hidden" name="deb-cred" value="crédito">
<input type="hidden" name="hc" value="<?=$hc?>">
